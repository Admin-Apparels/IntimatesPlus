...
export default function Poll() {
  ...
  const fetchPoll = useCallback(async () => {
    ...
    try {
      ...
      setTotalVotes(
        response.data.options.reduce((acc, option) => acc + option.votes, 0)
      );
      setFetching(false);
    } catch (error) {
      setFetching(false);
  ...
  const handleVote = async (index) => {
    ...
    try {
      await axios.post("/api/poll/vote", {
        option: poll.options[index].option,
      });
  ...
  return (
    <>
      ...
      <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          <ModalCloseButton />
          <ModalBody p={4} width={"100%"} borderColor="#934cce5e">
            ...
            {fetching ? (
              ...
              <>
                <Text fontSize="large" p={4} mb={"2"} textAlign="center">
                  {poll?.question}
                </Text>
                <Stack
                  display={"flex"}
          ...
          <ModalFooter>
            <Text textAlign={"center"} width={"100%"} p={"4"}>
              "Message us on the next poll, you'll be quoted."
            </Text>
          </ModalFooter>
        </ModalContent>
      </Modal>
